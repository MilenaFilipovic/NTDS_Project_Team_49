...
class Keywords(TextDataset):
    def __init__(self, **params):
        ...
        self.documents = np.array(dataset['keywords'])
        self.labels = np.array(dataset['first_genre_codes'])
        self.names = np.array(dataset['title'])
        self.class_names = params['categories']
        assert max(self.labels) + 1 == len(self.class_names)
        N, C = len(self.documents), len(self.class_names)
        print('N = {} documents, C = {} classes'.format(N, C))
        """
